...
export default class ChoicesController extends Controller {
  ...
  disconnect() {
    try {
      ...
      // element already gone, nothing left to tear down
    }
  }
  ...
  #choiceTemplate(template, choices, { classNames }, data) {
    ...
    return template(`
      ...
      ${value && desc
        ? `
        <div class="choices__item-description">
        ${desc}
        </div>
        `
        : ""
  ...
  // Opens the "add new" link in jet_ui's Dialog frame (data-turbo-frame="dialog"),
  // the current async-open convention. The original used "drawer" — still
  // works today via Dialog's legacy alias, but new code should target the
  // real frame name, not the deprecated one.
  #dropdownFooterTemplate() {
    return `
      <a href="${this.newPath}" data-turbo-frame="dialog" class="choices__item choices__item--choice choices__item-new" tabindex="-1">
